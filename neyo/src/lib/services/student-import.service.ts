...
export class ImportError extends Error {
  constructor(public code: "EMPTY" | "TOO_MANY_ROWS" | "BAD_FILE" | "NO_NAME_MAPPING" | "ABORTED", message: string) {
    super(message);
...
export async function commitImport(
  ...
  return withTenant(user.tenantId, async () => {
    ...
    for (const c of valid) {
      try {
        const admissionNo = c.admissionNo && c.admissionNo.length >= 3
          ? c.admissionNo
          : await nextTenantId(user.tenantId, "STUDENT");
