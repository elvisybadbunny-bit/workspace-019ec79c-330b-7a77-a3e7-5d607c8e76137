...
/**
 * Record a payment taken at the desk WITHOUT an STK push: cash, or an M-Pesa
 * payment the parent already made (manual reference entry). Marked PAID
 * immediately. Cash gets a synthetic ref so receipts always have one.
 */
export async function recordWalkInPayment(
  ...
  // Guard against re-entering the same M-Pesa receipt.
  if (input.method === "mpesa" && input.mpesaRef) {
    const dup = await db.payment.findUnique({ where: { mpesaRef: input.mpesaRef } });
    if (dup) throw new ReceptionError("DUPLICATE", "That M-Pesa reference is already recorded.");
  }

  const ref =
    input.method === "mpesa" && input.mpesaRef
      ? input.mpesaRef
  ...
  const payment = await db.payment.create({
    data: {
      tenantId,
      provider: input.method === "mpesa" ? "mpesa_manual" : "cash",
      amount: input.amount,
      phone: input.phone,
