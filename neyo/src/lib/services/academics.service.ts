...
export async function listDepartments(user: SessionUser) {
  return withTenant(user.tenantId, async () => {
    const rows = await tenantDb().department.findMany({ orderBy: { name: "asc" }, include: { subjects: { where: { archived: false } } } });
    const hodIds = rows.map((d) => d.hodId).filter((x): x is string => Boolean(x));
    const hods = hodIds.length ? await tenantDb().user.findMany({ where: { id: { in: hodIds } }, select: { id: true, fullName: true } }) : [];
...
export async function setSlot(user: SessionUser, input: { classId: string; subjectId: string; teacherId?: string; dayOfWeek: number; period: number }) {
  return withTenant(user.tenantId, async () => {
    ...
    const row = await db.timetableSlot.upsert({
      ...
      create: { tenantId: user.tenantId, classId: input.classId, subjectId: input.subjectId, teacherId: input.teacherId || null, dayOfWeek: input.dayOfWeek, period: input.period, slotType: "ACADEMIC" },
      update: { subjectId: input.subjectId, teacherId: input.teacherId || null },
    });
