...
export class FinanceError extends Error {
  constructor(public code: "NOT_FOUND" | "DUPLICATE" | "EMPTY", message: string) {
    super(message);
...
export async function listStructures(user: SessionUser) {
  return withTenant(user.tenantId, async () => {
    ...
    return rows.map((s) => ({
      id: s.id, name: s.name, level: s.level, year: s.year, term: s.term,
...
export async function createStructure(user: SessionUser, input: { level: string; year: number; term: number; items: { label: string; amountKes: number }[] }) {
  return withTenant(user.tenantId, async () => {
    const dup = await tenantDb().feeStructure.findFirst({ where: { level: input.level, year: input.year, term: input.term } });
    if (dup) throw new FinanceError("DUPLICATE", `A fee structure for ${input.level}, Term ${input.term} ${input.year} already exists.`);
    const s = await tenantDb().feeStructure.create({
      data: {
        name: `${input.level} — Term ${input.term} ${input.year}`,
        level: input.level, year: input.year, term: input.term,
...
export async function batchInvoice(user: SessionUser, structureId: string, dueDate: string) {
  return withTenant(user.tenantId, async () => {
    ...
    const classes = await tenantDb().schoolClass.findMany({ where: { level: structure.level, archived: false } });
  ...
  return withTenant(user.tenantId, async () => {
    ...
    const inv = await tenantDb().invoice.create({
      data: {
        ...
        totalKes: input.totalKes, dueDate: input.dueDate, year: input.year, term: input.term,
      } as never,
    });
...
export async function listInvoices(user: SessionUser, filters: { status?: string; q?: string }) {
  return withTenant(user.tenantId, async () => {
    ...
    const visible = await tenantDb().student.findMany({ where: scope, select: { id: true, firstName: true, middleName: true, lastName: true, admissionNo: true, schoolClass: { select: { level: true, stream: true } } } });
    ...
    if (filters.q) {
      ...
      rows = rows.filter((r) => {
        ...
        return r.invoiceNo.toLowerCase().includes(q) || name.includes(q) || (s?.admissionNo.toLowerCase().includes(q) ?? false);
      });
    }
    return rows.map((r) => {
      const s = vMap.get(r.studentId);
