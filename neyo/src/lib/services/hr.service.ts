...
export async function upsertProfile(user: SessionUser, input: { userId: string; tscNumber?: string; nationalId?: string; kraPin?: string; qualifications?: string; employmentDate?: string; contractType?: string; contractEndDate?: string; emergencyContact?: string }) {
  return withTenant(user.tenantId, async () => {
...
export async function promoteStaff(user: SessionUser, targetUserId: string, newRole: string, note?: string) {
  return withTenant(user.tenantId, async () => {
    ...
    await audit(user, "hr.staff_promoted", "user", targetUserId, { from: oldRole, to: newRole, note });
    return { userId: targetUserId, from: oldRole, to: newRole };
  });
}
