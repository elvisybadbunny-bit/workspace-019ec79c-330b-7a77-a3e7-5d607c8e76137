...
/** Can this user touch this class? (CLASS_TEACHER/TEACHER = own classes only.) */
async function assertClassInScope(user: SessionUser, classId: string) {
...
export async function getRegister(user: SessionUser, classId: string, date: string) {
  return withTenant(user.tenantId, async () => {
    const cls = await assertClassInScope(user, classId);
...
export async function markRegister(user: SessionUser, input: MarkRegisterInput) {
  return withTenant(user.tenantId, async () => {
    await assertClassInScope(user, input.classId);
    ...
    await db.auditLog.create({
      data: {
        ...
        metadata: JSON.stringify({ date: input.date, ...counts, total: marks.length }),
      },
    });
...
export async function attendanceOverview(user: SessionUser, date: string) {
  return withTenant(user.tenantId, async () => {
    ...
    if (role === "TEACHER" || role === "CLASS_TEACHER") classWhere.classTeacherId = user.id;
