...
async function assertStudentInScope(user: SessionUser, studentId: string) {
  ...
  if (role === "TEACHER" || role === "CLASS_TEACHER" || role === "HOD" || role === "DEAN_OF_STUDIES") {
...
export async function reportIncident(
  ...
  return withTenant(user.tenantId, async () => {
    ...
    // AUTO PARENT SMS for MAJOR/SEVERE (B.20.5).
    ...
    await audit(user, "discipline.incident_reported", "disciplineIncident", incident.id, {
      student: incident.studentName, category: input.category, severity: input.severity, points, parentNotified,
    });
    return { id: incident.id, points, parentNotified };
  });
}

export async function listIncidents(user: SessionUser, q: { studentId?: string } = {}) {
  return withTenant(user.tenantId, async () => {
    ...
    return tenantDb().disciplineIncident.findMany({
      where: { ...studentFilter, ...(q.studentId ? { studentId: q.studentId } : {}) },
...
export async function behaviorBoard(user: SessionUser) {
  return withTenant(user.tenantId, async () => {
    ...
    const incidents = await tenantDb().disciplineIncident.findMany({ where: { date: { gte: `${year}-01-01` } } });
