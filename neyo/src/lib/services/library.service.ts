...
export function computeFine(dueDate: string, onDate = nairobiToday()): number {
  return overdueDays(dueDate, onDate) * FINE_PER_DAY_KES;
}
...
export async function issueBook(user: SessionUser, input: { bookId: string; studentId: string; dueDate: string }) {
  return withTenant(user.tenantId, async () => {
    ...
    const issue = await db.bookIssue.create({
      data: {
        tenantId: user.tenantId, bookId: book.id, studentId: student.id,
