...
import { REDIS_ENABLED } from "@/lib/jobs/jobs.service";
...
export async function runHealthChecks(): Promise<{
  ...
  checks.push({
    ...
    status: REDIS_ENABLED ? "operational" : "not_configured",
    detail: REDIS_ENABLED ? undefined : "Running in-process (no Redis yet)",
  });
  checks.push({
    name: "File storage",
