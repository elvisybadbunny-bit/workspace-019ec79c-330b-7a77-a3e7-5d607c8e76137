...
function RoomBoard({ hostel, canManage, onBack }: { hostel: HostelRow; canManage: boolean; onBack: () => void }) {
  ...
  React.useEffect(() => {
    ...
    fetch("/api/students?status=ACTIVE").then((r) => r.json()).then((j) => {
      if (j.ok) setStudents(j.data.students.map((s: { id: string; firstName: string; middleName?: string | null; lastName: string; admissionNo: string; gender: string }) => ({
        id: s.id, name: [s.firstName, s.middleName, s.lastName].filter(Boolean).join(" "), admissionNo: s.admissionNo, gender: s.gender,
      })));
    }).catch(() => {});
  }, [load]);
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-navy-950/40 p-4 backdrop-blur-sm sm:items-center" onClick={onClose}>
      <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-card dark:bg-navy-900" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-3">
          <div>
            <Label>Student</Label>
            <select value={studentId} onChange={(e) => setStudentId(e.target.value)} className="w-full rounded-2xl border border-navy-200 bg-white px-3.5 py-2.5 text-sm dark:border-navy-700 dark:bg-navy-900">
              <option value="">Pick a student…</option>
              {eligible.map((s) => <option key={s.id} value={s.id}>{s.name} — {s.admissionNo}</option>)}
            </select>
