...
export function DisciplineClient({ canManage, canConfidential }: { canManage: boolean; canConfidential: boolean }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {tab === "suspensions" && (
        <div className="space-y-3">
          {canConfidential && <Button onClick={() => setDialog("suspend")}><Plus className="h-4 w-4" /> Issue suspension</Button>}
                ...
                <Card key={s.id}>
                  <CardContent className="flex flex-wrap items-center justify-between gap-2 p-4">
                    <div className="space-y-0.5">
                      ...
                      <p className="text-xs text-navy-400">{s.startDate} → {s.endDate} · {s.reason}</p>
...
function SuspendDialog({ students, onClose, onDone }: { students: StudentOpt[]; onClose: () => void; onDone: () => void }) {
  ...
  async function save() {
    ...
    try {
      ...
      const json = await res.json();
      if (json.ok) {
        toast({ title: json.data.parentNotified ? "Suspension issued — parent SMS sent" : "Suspension issued", tone: "success" });
        onDone();
      } else toast({ title: json.error?.message || "Failed", tone: "error" });
    } finally { setSaving(false); }
  }
  ...
  return (
    <Dialog title="Issue a suspension" onClose={onClose}>
      <div className="space-y-3">
        <StudentSelect students={students} value={f.studentId} onChange={(v) => set("studentId", v)} />
        <div className="grid grid-cols-2 gap-3">
        ...
        <p className="rounded-xl bg-amber-50 px-3 py-2 text-xs text-amber-800 dark:bg-amber-900/20 dark:text-amber-300">The parent is notified by SMS immediately.</p>
        <Button onClick={save} disabled={saving || !f.studentId || !f.endDate || f.reason.trim().length < 5} className="w-full">
          {saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <UserX className="h-4 w-4" />} Issue suspension
        </Button>
      </div>
    </Dialog>
  );
