    ...
    <div className="space-y-4">
      ...
      <Card>
        ...
        <CardContent className="flex flex-wrap gap-2">
          {data.contacts.map((t) => (
            <a key={t.id} href={`/messages?to=${t.id}`}>
              <Button size="sm" variant="secondary">{t.fullName} · {t.role === "PRINCIPAL" ? "Principal" : t.role === "DEPUTY_PRINCIPAL" ? "Deputy" : "Class teacher"}</Button>
            </a>
          ))}
        </CardContent>
      </Card>
