...
interface Structure { id: string; name: string; level: string; year: number; term: number; totalKes: number; items: { id: string; label: string; amountKes: number }[]; invoiceCount: number }
...
function OverviewTab() {
  ...
  const load = React.useCallback(async () => {
    setError(false);
    try {
      const res = await fetch("/api/finance/invoices?aging=1");
      const json = await res.json();
      if (json.ok) setAging(json.data); else setError(true);
    } catch { setError(true); }
  }, []);
            ...
            <Card key={s.id}>
              <CardHeader>
                <CardTitle className="flex items-center justify-between">
                  <span>{s.name}</span>
                  <Badge tone="neutral">{kes(s.totalKes)}</Badge>
                </CardTitle>
              </CardHeader>
...
function StructureDialog({ onClose, onDone }: { onClose: () => void; onDone: () => void }) {
  ...
  async function save() {
    ...
    try {
      const res = await fetch("/api/finance/structures", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ level, year, term, items: items.filter((i) => i.label && i.amountKes).map((i) => ({ label: i.label, amountKes: Number(i.amountKes) })) }),
      });
      const json = await res.json();
      if (json.ok) onDone();
      else toast({ title: json.error?.message || "Failed", tone: "error" });
...
function BatchDialog({ structure, onClose, onDone }: { structure: Structure; onClose: () => void; onDone: (msg: string) => void }) {
  ...
  return (
    <Modal title={`Invoice all of ${structure.level}`} onClose={onClose}>
      <div className="space-y-3">
        <p className="rounded-xl bg-warm-50 px-3 py-2 text-sm text-navy-600 dark:bg-navy-800 dark:text-navy-300">
          Every active {structure.level} student gets a <span className="font-semibold">{kes(structure.totalKes)}</span> invoice for {structure.name}. Students already invoiced are skipped.
        </p>
              ...
              <TBody>
                {promises.map((p) => (
                  <TR key={p.id}>
                    ...
                    <TD className="font-mono text-navy-700 dark:text-navy-300">{p.promiseDate}</TD>
                    <TD>
                      <div>
