...
import {
  ...
  Loader2, ArrowLeft, MessageSquareWarning, Users, WifiOff,
} from "lucide-react";
...
interface OverviewClass { id: string; label: string; total: number; marked: number; present: number; absent: number; done: boolean }
    ...
    <div className="space-y-6">
      ...
      {error ? (
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {classes.map((c) => (
            <button key={c.id} onClick={() => setOpenClass(c.id)} className="text-left">
...
function Register({ classId, date, canRecord, onBack }: {
  classId: string; date: string; canRecord: boolean; onBack: () => void;
}) {
