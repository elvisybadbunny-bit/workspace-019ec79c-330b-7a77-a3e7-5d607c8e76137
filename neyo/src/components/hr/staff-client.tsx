          ...
          <TBody>
            {filtered.map((r) => (
              <TR key={r.userId}>
                ...
                <TD><Button size="sm" variant="secondary" onClick={() => setFile(r.userId)}><FileText className="h-3.5 w-3.5" /> File</Button></TD>
              </TR>
            ))}
          </TBody>
        </Table>
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-navy-950/40 backdrop-blur-sm" onClick={onClose}>
      <div className="h-full w-full max-w-lg overflow-y-auto bg-white p-6 shadow-card dark:bg-navy-900" onClick={(e) => e.stopPropagation()}>
        ...
        {promote && data && <PromoteDialog userId={userId} current={data.staff.role} onClose={() => setPromote(false)} onDone={() => { setPromote(false); load(); toast({ title: "Role updated", tone: "success" }); }} />}
...
function PromoteDialog({ userId, current, onClose, onDone }: { userId: string; current: string; onClose: () => void; onDone: () => void }) {
  ...
  return (
    <Modal title="Change role / promote" onClose={onClose}>
      <div className="space-y-3">
        <div>
        ...
        <div><Label>Note (goes to the audit trail)</Label><Input value={note} onChange={(e) => setNote(e.target.value)} placeholder="e.g. Promoted to HOD Sciences" /></div>
        <Button onClick={save} disabled={saving || newRole === current} className="w-full">{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <ArrowUpRight className="h-4 w-4" />} Update role</Button>
      </div>
    </Modal>
  );
}
