...
interface Student {
  id: string; admissionNo: string; firstName: string; middleName: string | null; lastName: string;
        ...
        <CardContent className="flex flex-col gap-4 p-6 sm:flex-row sm:items-center">
          ...
          <div className="flex-1">
            ...
            <div className="mt-1 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-navy-500 dark:text-navy-400">
              <span className="inline-flex items-center gap-1 font-mono text-xs"><Hash className="h-3.5 w-3.5" />{s.admissionNo}</span>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card>
          ...
          <CardContent className="space-y-2 text-sm">
            <Row label="Admission no." value={s.admissionNo} mono />
          ...
          <CardContent className="space-y-3">
            {s.guardians.length === 0 ? (
              ...
              <div key={g.id} className="rounded-xl border border-navy-100 p-3 dark:border-navy-800">
                ...
                <div className="mt-1 flex flex-wrap gap-x-4 text-xs text-navy-500 dark:text-navy-400">
...
function AddDoc({ studentId, onAdded }: { studentId: string; onAdded: ()=>void }) {
  ...
  return open ? (
    <div className="flex items-center gap-2">
      <Input value={label} onChange={(e)=>setLabel(e.target.value)} placeholder="Label e.g. Birth cert" className="h-8 w-40" />
      <FileUpload category="student-doc" label="File" onUploaded={async (file)=>{
        const res = await fetch(`/api/students/${studentId}/documents`, { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ label: label || file.fileName, fileUrl: file.url, fileName: file.fileName }) });
        const json = await res.json();
        if (json.ok) { toast({title:"Document added",tone:"success"}); setLabel(""); setOpen(false); onAdded(); }
        else toast({title:json.error?.message||"Failed",tone:"error"});
      }} />
    </div>
  ) : (
...
function AddGuardian({ studentId, onAdded }: { studentId: string; onAdded: () => void }) {
  ...
  return (
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-navy-900/40 backdrop-blur-sm sm:items-center sm:p-4" onClick={() => setOpen(false)}>
