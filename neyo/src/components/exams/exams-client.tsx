...
export function ExamsClient({ canManage, canEnterMarks, canPublish }: { canManage: boolean; canEnterMarks: boolean; canPublish: boolean }) {
  ...
  if (open) {
    return <ExamDetail exam={open} subjects={subjects} classes={classes} canEnterMarks={canEnterMarks} canPublish={canPublish} onBack={() => { setOpen(null); load(); }} />;
  }
...
function ExamDetail({ exam, subjects, classes, canEnterMarks, canPublish, onBack }: {
  exam: ExamRow; subjects: Subject[]; classes: ClassOpt[];
  canEnterMarks: boolean; canPublish: boolean; onBack: () => void;
}) {
  const { toast } = useToast();
        ...
        <div className="flex items-center gap-2">
          ...
          {canPublish && (
            <Button size="sm" variant={published ? "secondary" : "primary"} onClick={togglePublish}>
              ...
              {published ? "Unpublish" : "Release results"}
            </Button>
          )}
        </div>
      </div>
