...
async function loadSite(): Promise<PublicSite> {
  const slug = currentTenantSlug();
  if (!slug) redirect("/login");
  return publicSiteBySlug(slug);
}

export async function generateMetadata(): Promise<Metadata> {
  const slug = currentTenantSlug();
  if (!slug) return { title: "NEYO" };
  try {
    const site = await publicSiteBySlug(slug);
    const title = site.settings.seoTitle || `${site.school.name} — Admissions and school updates`;
    const description = site.settings.seoDescription || site.school.about || `Learn about ${site.school.name}, admissions, academics and parent services.`;
    const image = site.settings.ogImageUrl || site.settings.heroImageUrl || site.school.logoUrl || undefined;
    return {
      title,
      description,
      openGraph: {
    ...
    return { title: "School website — NEYO" };
  }
}

/** G.11 — Public School Landing Site on Subdomain. */
